...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_poi') {
    ...
    if ($_POST['action'] === 'update_banner_peta') {
        $uploaded = handle_image_upload('banner_foto');
        if ($uploaded) {
            $stmt = $pdo->prepare("INSERT INTO settings (key_name, key_value) VALUES ('header_peta', ?) ON DUPLICATE KEY UPDATE key_value = VALUES(key_value)");
            $stmt->execute([$uploaded]);
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Foto banner Peta Kelurahan berhasil diperbarui.'];
        }
        header("Location: peta.php");
        exit;
    } elseif ($_POST['action'] === 'reset_banner_peta') {
        $stmt = $pdo->prepare("INSERT INTO settings (key_name, key_value) VALUES ('header_peta', '') ON DUPLICATE KEY UPDATE key_value = VALUES(key_value)");
        $stmt->execute();
        $_SESSION['flash'] = ['type' => 'success', 'message' => 'Banner Peta Kelurahan dikembalikan ke background default.'];
        header("Location: peta.php");
        exit;
    }
}

$stmtBanner = $pdo->query("SELECT key_value FROM settings WHERE key_name = 'header_peta'");
$banner_peta = $stmtBanner->fetchColumn() ?: '';
  ...
  <main class="admin-main">
    ...
    <div class="section-card" style="margin-bottom: 24px;">
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 16px;">
            <div>
                <h2 style="font-size: 1.1rem; margin: 0 0 4px 0;"><i class="fa-solid fa-image" style="color: var(--teal-600);"></i> Banner Header Peta</h2>
                <p style="margin: 0; font-size: 0.85rem; color: var(--ink-soft);">Ganti foto latar belakang header pada halaman Peta Kelurahan publik.</p>
            </div>
        </div>
        ...
        <div style="margin-top: 16px; display: flex; flex-wrap: wrap; gap: 16px; align-items: center;">
            <div style="flex: 1; min-width: 260px; height: 130px; border-radius: 12px; overflow: hidden; background: #ecfdf5; border: 1px solid #e2e8f0; position: relative;">
                <?php if (!empty($banner_peta)): ?>
                    <img src="../<?= htmlspecialchars($banner_peta) ?>" style="width: 100%; height: 100%; object-fit: cover;" alt="Banner Peta">
                <?php else: ?>
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                <form method="POST" enctype="multipart/form-data" style="margin:0;">
                    <input type="hidden" name="action" value="update_banner_peta">
                ...
                <form method="POST" style="margin:0;">
                    <input type="hidden" name="action" value="reset_banner_peta">
